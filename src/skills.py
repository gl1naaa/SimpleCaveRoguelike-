...
class Skill:
    ...
    scale_stat: Optional[str] = None  # 'str', 'agi', 'int' or None
    scale_coef: float = 0.3  # stat scaling coefficient
...
_reg(Skill("sword_0", "Cleave",
    ...
    "swordsman", "sword", 0, 0.4, 8, 20, 1, aoe=3, scale_stat="str", scale_coef=0.3))

_reg(Skill("sword_1", "Whirlwind",
    "Spin attack, hits all enemies in 3x3",
    "swordsman", "sword", 1, 1.2, 14, 14, 1, aoe=1, scale_stat="str", scale_coef=0.3))

_reg(Skill("sword_2", "Shield Bash",
    "Bash enemy, stun 1.5s + knockback",
    "swordsman", "sword", 2, 2.0, 12, 8, 1, effect="stun", effect_duration=1.5, scale_stat="str", scale_coef=0.3))
...
_reg(Skill("archer_0", "Quick Shot",
    ...
    "archer", "bow", 0, 0.5, 5, 9, 8, scale_stat="agi", scale_coef=0.3))

_reg(Skill("archer_1", "Volley",
    "3 arrows in a spread",
    "archer", "bow", 1, 1.0, 12, 10, 5, aoe=3, scale_stat="agi", scale_coef=0.3))

_reg(Skill("archer_2", "Poison Arrow",
    "Poison DOT for 4s, -3 dmg/tick",
    "archer", "bow", 2, 2.5, 10, 10, 6, effect="burn", effect_duration=4.0, effect_power=3, scale_stat="agi", scale_coef=0.3))

_reg(Skill("archer_3", "Frost Arrow",
    "Slow enemy 60% for 3s",
    "archer", "bow", 3, 3.0, 8, 10, 6, effect="slow", effect_duration=3.0, effect_power=60, scale_stat="agi", scale_coef=0.3))

_reg(Skill("archer_4", "Arrow Rain",
    "Massive 5x5 AoE barrage",
    "archer", "bow", 4, 4.5, 28, 7, 10, aoe=2, scale_stat="agi", scale_coef=0.3))
...
_reg(Skill("mage_0", "Fireball",
    ...
    "mage", "staff", 0, 2.0, 16, 14, 12, effect="burn", effect_duration=3.0, effect_power=4, scale_stat="int", scale_coef=0.4))

_reg(Skill("mage_1", "Chain Lightning",
    "Lightning bounces to 3 targets",
    "mage", "staff", 1, 3.0, 14, 12, 14, aoe=3, scale_stat="int", scale_coef=0.4))

_reg(Skill("mage_2", "Blizzard",
    "5x5 ice storm, slow 50% 2s",
    "mage", "staff", 2, 5.5, 22, 9, 16, aoe=2, effect="slow", effect_duration=2.0, effect_power=50, scale_stat="int", scale_coef=0.4))
...
_reg(Skill("summon_1", "Spirit Drain",
    ...
    "summoner", "orb", 1, 2.5, 14, 6, 1, effect="lifesteal", effect_duration=1.0, effect_power=50, scale_stat="int", scale_coef=0.3))

_reg(Skill("summon_2", "Fire Nova",
    "AoE fire explosion around self",
    "summoner", "orb", 2, 2.0, 18, 0, 3, aoe=1, effect="burn", effect_duration=2.0, effect_power=5, scale_stat="int", scale_coef=0.3))
...
_reg(Skill("heal_0", "Holy Light",
    ...
    "healer", "mace", 0, 1.5, 10, 0, 0, effect="heal", effect_power=15, scale_stat="int", scale_coef=0.3))

_reg(Skill("heal_1", "Greater Heal",
    "Powerful self heal (30% HP)",
    "healer", "mace", 1, 5.0, 22, 0, 0, effect="heal", effect_power=30, scale_stat="int", scale_coef=0.3))

_reg(Skill("heal_2", "Radiance",
    "AoE heal allies in radius (12% HP)",
    "healer", "mace", 2, 8.0, 26, 0, 0, effect="aoe_heal", effect_power=12, scale_stat="int", scale_coef=0.3))

_reg(Skill("heal_3", "Blessing",
    "Buff self + allies +40% ATK for 8s",
    "healer", "mace", 3, 10.0, 32, 0, 0, effect="ally_buff", effect_power=40, effect_duration=8.0, scale_stat="int", scale_coef=0.3))

_reg(Skill("heal_4", "Divine Surge",
    "Heal self 18% HP + restore 60 MP",
    "healer", "mace", 4, 12.0, 40, 0, 0, effect="heal", effect_power=18, mana_regen=60, scale_stat="int", scale_coef=0.3))
...
_reg(Skill("rogue_0", "Backstab",
    ...
    "rogue", "dagger", 0, 0.8, 4, 25, 1, effect="crit_boost", effect_power=100, scale_stat="agi", scale_coef=0.3))

_reg(Skill("rogue_1", "Shadow Strike",
    "Teleport behind enemy + hit (20 dmg, +50% crit)",
    "rogue", "dagger", 1, 3.0, 10, 20, 6, effect="crit_boost", effect_power=50, scale_stat="agi", scale_coef=0.3))

_reg(Skill("rogue_2", "Fan of Knives",
    "AoE 3x3, each target gets 15 dmg + separate crit",
    "rogue", "dagger", 2, 2.0, 8, 15, 1, aoe=1, scale_stat="agi", scale_coef=0.3))

_reg(Skill("rogue_3", "Poison Blade",
    "Hit 15 dmg + burn DOT 6dmg/s for 5s",
    "rogue", "dagger", 3, 3.0, 7, 15, 1, effect="burn", effect_duration=5.0, effect_power=6, scale_stat="agi", scale_coef=0.3))
...
def get_skill_for_slot(class_name: str, slot: int) -> Optional[Skill]:
    ...
    # Each actor needs an independent cooldown timer. Returning the registry
    # object directly made one NPC/player's skill cooldown affect everyone.
    skill = ALL_SKILLS.get(skill_id)
    return copy.deepcopy(skill) if skill is not None else None
